...
run() {
    ...
    for i in $(seq 0 $NUM_CHUNKS_IN_BATCH 1000); do
        ...
        if [ $result != 0 ]; then
            ...
            echo "NODE_OPTIONS=\"-r ts-node/register --no-warnings --max-old-space-size=4000\" node ./rewrite.ts -i $INPUT -o $OUTPUT -s $i -c $NUM_CHUNKS_IN_BATCH"
            echo 
            continue
        fi
